...
// LOCAL FIX (not upstream): in C++ these first three are BITFIELDS packed into one u32
//   unsigned int Colored : 1; unsigned int Visible : 1; unsigned int Codepoint : 30;
// but the generator emitted them as three separate `c.uint`s. That made the struct 8 bytes too long
// and shifted EVERY field after it: reading `.Y0` handed you the real Y1, and `.X1`/`.Y1` handed you
// texture UVs. It is silent - the values are plausible floats - and it is what made gui_draw_icon
// place every toolbar icon ~2px too high. Re-declared as a bit_field so the layout matches C++ again.
// Re-apply this if lib/odin-imgui is ever regenerated.
FontGlyph :: struct {
	using _:   FontGlyphPacked,
...
// The packed head of ImFontGlyph. See the note on FontGlyph above.
FontGlyphPacked :: bit_field u32 {
	Colored:   u32 | 1,  // Flag to indicate glyph is colored and should generally ignore tinting
	Visible:   u32 | 1,  // Flag to indicate glyph has no visible pixels (e.g. space)
	Codepoint: u32 | 30, // 0x0000..0x10FFFF
}
#assert(size_of(FontGlyph) == 40) // 4 packed + 9 floats; 48 means the bitfield regressed
