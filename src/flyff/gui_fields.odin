...
// --- editing a comma-joined name list ---------------------------------------------------------------
//
...
// text and the value is the comma-joined badge list - which is why a list argument is no longer
...
ed_suggest_field :: proc(
  ...
  typed := imgui.InputTextWithHint(label, hint, cstring(raw_data(buffer)), len(buffer))
